# Web App com Inteligência Artificial Para Análise e Previsão de Preços de Ações
...
st.sidebar.info('VPL_Consultoria')
...
@st.cache_resource
def vpl_download_dados(ticker, start_date, end_date):
...
option = st.sidebar.selectbox("Selecione a Empresa", ["Apple", "Google", "Microsoft", "Tesla", "Netflix", "Meta"])
...
# Download dos dados
df_vpl_dados = vpl_download_dados(stock,start_date,end_date)
...
def vpl_indicadores_tecnicos():
    ...
    indicador_bb = BollingerBands(df_vpl_dados.Close)
    ...
    macd_object = MACD(df_vpl_dados['Close'])

    # Calcula o indicador RSI para o preço de fechamento
    rsi = RSIIndicator(df_vpl_dados.Close).rsi()
    
    # Calcula a Média Móvel Simples (SMA) para o preço de fechamento
    sma = SMAIndicator(df_vpl_dados.Close, window=14).sma_indicator()
    
    # Calcula a Média Móvel Exponencial (EMA) para o preço de fechamento
    ema = EMAIndicator(df_vpl_dados.Close).ema_indicator()
    ...
    if option == 'Preço de Fechamento':
        ...
        st.line_chart(df_vpl_dados.Close)
        ...
        df_vpl_dados['MACD'] = macd_object.macd()
        df_vpl_dados['Signal_Line'] = macd_object.macd_signal()
        df_vpl_dados['MACD_Diff'] = macd_object.macd_diff()
        ...
        plt.subplot(2, 1, 1)
        plt.plot(df_vpl_dados['Close'], label='Close Price')
        plt.legend()

        # Plot do MACD
        plt.subplot(2, 1, 2)
        plt.plot(df_vpl_dados['MACD'], label='MACD', color='blue')
        plt.plot(df_vpl_dados['Signal_Line'], label='Sinal', color='magenta')
        plt.bar(df_vpl_dados.index, df_vpl_dados['MACD_Diff'], label='Histograma', color='black', alpha=0.8)
        plt.legend()
        st.pyplot(plt)
...
def vpl_imprime_tabela():
    st.header('Tabela de Dados')
    st.dataframe(df_vpl_dados.tail(20))
...
def vpl_inteligencia_artificial(num):
    ...
    modelo_vpl = LinearRegression().fit(x_train_scaled, y_train)
    
    # Faz previsões com os dados de teste padronizados
    preds = modelo_vpl.predict(x_test_scaled)
    ...
    forecast_pred = modelo_vpl.predict(x_forecast_scaled)
...
st.caption('VPL Consultoria.')

# Bloco principal do programa Python
if __name__ == '__main__':
        vpl_seleciona_pagina()
